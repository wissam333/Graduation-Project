...
const userRecommendations = async (req, res) => {
  try {
    ...
    const orders = await Order.find({ userId: userObjectId }).lean();
    ...
    // Convert userProductIds to ObjectIds
    const userProductObjectIds = Array.from(userProductIds).map(
    ...
    const similarUsers = await Order.aggregate([
      {
        $match: {
          "products.productId": { $in: userProductObjectIds },
    ...
    // Convert similar user IDs to ObjectIds
    const similarUserObjectIds = similarUsers.map((u) => u._id);

    // Get products ordered by similar users that current user hasn't ordered
    const recommendedProducts = await Order.aggregate([
      { $match: { userId: { $in: similarUserObjectIds } } },
      { $unwind: "$products" },
      {
        $match: {
          "products.productId": { $nin: userProductObjectIds },
        },
      },
