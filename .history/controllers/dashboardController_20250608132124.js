...
const getDashboardStats = async (req, res) => {
  try {
    ...
    // Check if user is admin (show all restaurants) or manager (show only their restaurant)
    const isAdmin = req.user.role === "0";

    // Create cache key based on user role and restaurant
    const cacheKey = isAdmin
      ? "DashboardStats_Admin"
      : `DashboardStats_${restaurantId}`;
    ...
    // Retrieve counts
    const [ordersCount, productsCount, categoriesCount, activeManagers] =
      await Promise.all([
        Order.countDocuments(baseQuery),
        Product.countDocuments(baseQuery),
        Category.countDocuments(baseQuery),
        isAdmin
          ? User.countDocuments({ role: "2" }) // Count all managers for admin
          : User.countDocuments({ restaurantId, role: "2" }), // Count managers for specific restaurant
      ]);
    ...
    cache.put(cacheKey, stats, 10 * 60 * 1000);
