...
const getOrderById = async (req, res) => {
  try {
    ...
    // Populate user, products, and restaurants
    const [user, populatedProducts] = await Promise.all([
      ...
      Promise.all(
        order.products.map(async (item) => {
          const product = await Product.findById(item.productId).populate({
            path: "restaurantId",
            model: "Restaurant", // Make sure to use your actual Restaurant model name
          });
          ...
          return {
            ...
            product: product
              ? {
                  ...product.toObject(),
                  restaurant: product.restaurantId, // Add restaurant document
                }
              : null,
          };
        })
...
const getAllOrders = async (req, res) => {
  try {
    ...
    // Populate users, products, and restaurants for all orders
