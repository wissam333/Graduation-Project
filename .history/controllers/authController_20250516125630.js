...
const registerManager = async (req, res) => {
  try {
    ...
    // Verify requesting user has permission (only restaurant owner or admin can add managers)
    if (req.user.role !== "0" && !restaurant.createdBy.equals(req.user._id)) {
      return res.status(403).json({
        success: false,
        message: "You don't have permission to add managers to this restaurant",
      });
    }
    ...
    const newManager = new User({
      ...
      role: "2", // Set role to manager
      managedRestaurants: [restaurantId], // Assign to restaurant
    });
    ...
    restaurant.managers.push(savedManager._id);
    await restaurant.save();

    // Generate access token
    const accessToken = jwt.sign(
      { id: savedManager._id, role: savedManager.role },
      process.env.JWT_SEC,
      { expiresIn: "3d" }
    );

    // Return response without password
    const { password: _, ...managerWithoutPassword } = savedManager.toObject();

    res.status(201).json({
      success: true,
      message: "Manager registered and assigned to restaurant successfully",
      data: {
