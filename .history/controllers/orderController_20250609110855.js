...
const generateInvoice = async (req, res) => {
  try {
    ...
    // Here you have options for how to return the invoice:
    // Option 1: Return JSON data (frontend can generate PDF)
    // res.status(200).json(invoiceData);

    // Option 2: Generate PDF on server and send as file (requires PDF library)
    // For this example, we'll implement Option 2 using pdfkit

    // Install pdfkit first: npm install pdfkit
    ...
    doc.fontSize(12).text(`Name: ${invoiceData.customer.name}`);
    ...
    doc.text("Item", 50, doc.y, { width: 200, align: "left" });
    doc.text("Qty", 250, doc.y, { width: 50, align: "right" });
    doc.text("Price", 300, doc.y, { width: 80, align: "right" });
    ...
    invoiceData.items.forEach((item) => {
      doc.text(item.name, 50, doc.y, { width: 200, align: "left" });
      doc.text(item.quantity.toString(), 250, doc.y, {
        width: 50,
        align: "right",
      });
      doc.text(`$${item.unitPrice.toFixed(2)}`, 300, doc.y, {
        width: 80,
        align: "right",
      });
      ...
      doc.text(`From: ${item.restaurant}`, 50, doc.y + 15, {
        width: 400,
        align: "left",
      });
      doc.moveDown();
    ...
    doc.text(`Delivery Fee: $${invoiceData.deliveryFee.toFixed(2)}`, {
      align: "right",
    });
    doc.text(`Total: $${invoiceData.total.toFixed(2)}`, { align: "right" });
    doc.moveDown();
