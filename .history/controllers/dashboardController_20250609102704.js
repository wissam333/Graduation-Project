...
const getDashboardStats = async (req, res) => {
  try {
    ...
    // Create ObjectId instance for restaurantId if it exists
    const restaurantObjectId = restaurantId
      ? new mongoose.Types.ObjectId(restaurantId)
      : null;

    // Prepare common match stages for current and previous periods
    const commonCurrentMatch = {
      createdAt: { $gte: startDate, $lte: endDate },
    };
    const commonPreviousMatch = {
      createdAt: { $gte: previousStartDate, $lte: previousEndDate },
    };

    // Prepare restaurant filter if needed
    const restaurantFilter = !isAdmin
      ? [
          ...
          {
            $match: {
              "productDetails.restaurantId": restaurantObjectId,
            },
          },
        ]
      : [];

    // Main data aggregation - optimized to reduce duplicate operations
    const [currentData, previousData, counts, popularCategories] =
      await Promise.all([
        // Current period data - optimized facet with direct grouping
        Order.aggregate([
          { $match: commonCurrentMatch },
          ...restaurantFilter,
          {
            $group: {
              _id: null,
                              ...
                              [
                                {
                                  ...
                                  count: { $add: ["$$existingEntry.count", 1] },
                                },
                              ],
                            ],
                          },
        ...
        // Counts data - parallelized at database level
        Promise.all([
          Product.countDocuments(
            isAdmin ? {} : { restaurantId: restaurantObjectId }
          ),
        ...
        // Popular categories - optimized pipeline
        Order.aggregate([
          { $match: commonCurrentMatch },
          { $unwind: "$products" },
          {
          ...
          ...(!isAdmin
            ? [
                {
                  $match: { "productDetails.restaurantId": restaurantObjectId },
                },
              ]
            : []),
          {
    ...
    const currentSales = current.sales || { total: 0, count: 0 };
    ...
    const result = {
      ...
      charts: {
        salesTrend: current.byDay.map((day) => ({
          date: day._id,
          total: day.total,
          count: day.count,
        })),
