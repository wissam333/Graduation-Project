...
const getDashboardStats = async (req, res) => {
  try {
    ...
    // Since Order model doesn't have restaurantId, we need to adjust the base query
    // For non-admin users, we'll need to find orders containing products from their restaurant
    const baseQuery = isAdmin
      ? {}
      : {
          "products.productId": { $exists: true }, // This will be enhanced after product lookup
        };
    ...
    const [currentData, previousData, counts, popularCategories] =
      await Promise.all([
        ...
        Order.aggregate([
          ...
          // For non-admin users, we need to lookup products to filter by restaurant
          ...(!isAdmin
            ? [
                {
                  $lookup: {
        ...
        Order.aggregate([
          ...
          // For non-admin, filter by restaurant
          ...(!isAdmin
            ? [
                {
                  $match: {
