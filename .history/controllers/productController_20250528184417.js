...
const getProduct = async (req, res) => {
  try {
    ...
    const oneProduct = await Product.findById(productId)
      .populate("category", "-__v") // Changed from categoryId to category
      .populate("restaurant", "-__v"); // Changed from restaurantId to restaurant
...
const getProducts = async (req, res) => {
  try {
    const qCategory = req.query.category; // Changed from categoryId to category
    ...
    let query = {};
    if (qCategory) {
      query = { category: qCategory }; // Changed from categoryId to category
    }
    ...
    res.status(200).json({ products, totalPages, productsCount });
  } catch (err) {
    res.status(500).json(err);
  }
};
